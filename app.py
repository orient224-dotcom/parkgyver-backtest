...
# --- 1. 페이지 기본 설정 ---
st.set_page_config(page_title="박가이버 사령부 V10.39 (클리오 투입 황금레시피)", layout="wide", page_icon="🎛️")
...
st.sidebar.caption("은퇴 과수원 에디션 - 실전 투입 황금 레시피")
...
# 🌟 썩은 사과(테크윙) 방출 및 클리오 선발 투입
default_selected = ["삼성전자 (005930)", "실리콘투 (257720)", "제주반도체 (080220)", "HD현대일렉트릭 (267260)", "클리오 (237880)"]
...
# 🌟 파견 요원 1명 (풀베팅) 기본값
max_agents = st.sidebar.number_input("⚔️ 종목당 최대 파견 요원 수:", value=1, min_value=1, max_value=10)
...
# 🌟 최근 단기 트렌드 확인을 위해 기본 1년 설정 (수정 가능)
...
# 🌟 황금 레시피: 30% 레이더 가동, 7% 꺾임 청산
trailing_start_pct = st.sidebar.number_input("추적 레이더 가동 기준선 (%)", value=30.0, step=1.0)
...
for s_name, t_code in zip(names_list, tickers_list):
    # 코스피 대형주는 -2.5%, 코스닥/중소형주는 -5.0%를 기본값으로 세팅
...
# 🌟 황금 레시피: 심리적 MDD 방어를 위한 12% 숏컷
emergency_cut_pct = st.sidebar.number_input("비상 탈출 손실 기준선 (%)", value=12.0, step=1.0, min_value=5.0, max_value=50.0)
...
if run_btn or 'calculated' in st.session_state:
    ...
    with st.spinner("📡 [V10.39] 추세추종 엔진 및 현금선 추적 시뮬레이션 가동 중..."):
        ...
        if not combined_equity_df.empty:
            ...
            for idx, row in yearly_df.iterrows():
                ...
                y_html += f"<tr><td style='padding:7px;border:1px solid #eaeded;font-weight:bold;color:#8e44ad;'>{idx}</td><td style='padding:7px;border:1px solid #eaeded;'>{row['기초 자산']}</td><td style='padding:7px;border:1px solid #eaeded;'>{row['기말 자산']}</td><td style='padding:7px;border:1px solid #eaeded;color:{pnl_color};font-weight:bold;'>{row['실현 손익']}</td><td style='padding:7px;border:1px solid #eaeded;color:{ret_color};font-weight:bold;'>{row['계좌 총 수익률']}</td><td style='padding:7px;border:1px solid #eaeded;'>{row['매매 횟수']}</td></tr>"
