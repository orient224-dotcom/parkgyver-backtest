...
def get_stock_news(ticker):
    try:
        ...
        for n in items:
            ...
            if title: news_list.append(title.replace('&quot;', '"').replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>').replace('<b>', '').replace('</b>', ''))
        return news_list[:5] if news_list else ["주요 뉴스가 없습니다."]
    except: return ["통신 지연이 발생했습니다."]
...
with tab3:
    ...
    if st.button("📋 브리핑 생성 및 발송", use_container_width=True):
