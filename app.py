...
# =====================================================================
    ...
    buy_cond_input = st.sidebar.slider("🛒 진입 기준 (-% 하락 시)", 1, 20, 5, 1)
    stop_loss_input = st.sidebar.slider("🚨 강제 손절 기준 (-%)", 0, 50, 15, 1)
                ...
                for date, row in close_df.iterrows():
                    ...
                    for pos in active_positions:
                        ...
                        if t_code in row and not pd.isna(row[t_code]):
                            ...
                            if curr_price > pos.get('highest_price', pos['entry_price']):
                        ...
                        for pos in active_positions:
                            ...
                            if t_code in row and not pd.isna(row[t_code]):
                                ...
                                if stop_loss_limit is not None and gross_ret <= stop_loss_limit:
                                    ...
                                    drop_from_high = ((curr_price - pos['highest_price']) / pos['highest_price']) * 100
                                    if drop_from_high <= -float(trailing_stop_pct):
                                        is_exit = True
                                        exit_reason = f"📉 트레일링 스탑 (고점대비 {drop_from_high:.1f}%)"
