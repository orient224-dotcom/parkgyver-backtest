...
# --- 1. 페이지 웹 디자인 세팅 ---
st.set_page_config(page_title="박가이버 통합 작전 통제실 V5", page_icon="🛡️", layout="wide")

# --- 2. 사전 정의 종목 사전 (테마/섹터별 통합 DB) ---
SECTOR_DATABASE = {
    "⚡ 반도체 & HBM / 칩렛": {
        "테크윙": "089030.KQ",
        "한미반도체": "042700.KS",
...
# --- 3. 사이드바 최상단 [메뉴 선택 조종간] ---
st.sidebar.header("🎛️ 박가이버 사령부 메뉴")
...
if menu_choice == "🔎 1. 작전 구역(섹터) 탐색기":
    ...
    st.caption("관심 있는 테마별 종목을 둘러보고 담아 [작전 통제실 백테스트]로 보낼 수 있습니다.")
    st.markdown("---")
    ...
    if basket:
        ...
        if st.button("🚀 이 종목들로 [작전 통제실 백테스트] 즉시 실행!", type="primary"):
            st.session_state["selected_stocks"] = basket
            st.success(f"🎉 총 {len(basket)}개 종목이 사령부로 설정되었습니다!")
            st.info("👈 왼쪽 사이드바 메뉴에서 [🛡️ 2. 실전 작전 통제실 (백테스트)]를 클릭하시면 검증이 시작됩니다!")
    else:
        st.warning("⚠️ 최소 1개 이상의 종목을 선택해 주세요.")

# =====================================================================
# 🛡️ 모드 2: 실전 작전 통제실 (백테스트)
# =====================================================================
else:
    ...
    selected_stock_names = st.sidebar.multiselect(
        ...
        options=list(MASTER_STOCK_DICT.keys()),
        default=st.session_state["selected_stocks"],
        help="[탐색기] 메뉴에서 종목을 골라오시면 자동으로 반영됩니다."
    )
