...
if menu_choice == "🗄️ 1. 내 계좌 영구 DB (보유 & 관심)":
    ...
    <div class="hero-banner">
        ...
        <div class="hero-subtitle">스마트폰 전 종목 DB 연동으로 대한민국 상장 종목을 관리하세요!</div>
    </div>
    """, unsafe_allow_html=True)
    st.info("💡 사이드바와 메뉴를 통해 보유 종목 및 관심 종목을 세팅할 수 있습니다.")
    ...
    st.write(f"현재 감시 중인 주력 종목: {list(PORTFOLIO_UNIVERSE.keys())}")
            ...
            with st.spinner("📡 슈퍼컴퓨터가 백테스트를 가동 중입니다..."):
                try:
                    ...
                    for date, row in close_df.iterrows():
                        ...
                        asset_history.append({"Date": date_str, "Total_Asset": today_total_asset, "Drawdown": curr_dd})
