...
# --- 1. 페이지 기본 설정 ---
st.set_page_config(page_title="박가이버 사령부 V10.38 (정밀엔진 풀버전)", layout="wide", page_icon="🎛️")
...
# ⚡ [업그레이드 1] 데이터 고속 로딩을 위한 캐싱 (Speed Optimization)
...
st.sidebar.caption("은퇴 과수원 에디션 - 철통 자금관리 + 정밀 백테스트 엔진")
...
# 기울기 및 10일선 필터
use_trend_filter = st.sidebar.checkbox("📈 20일선 우상향(기울기) & 10일선 정배열 필터", value=True)
...
with live_col2:
    st.caption("💡 매일 오후 3시 20분 동시호가 시작 전 클릭하시면, 당일 등락률과 V10.38 자금 배분 룰 및 맞춤 타점을 적용해 즉시 실행할 지시서를 생성합니다.")

if scan_live_btn:
    if len(tickers_list) == 0:
        st.warning("⚠️ 종목을 먼저 선택해 주세요.")
    elif stock_alloc_pct * len(tickers_list) > 100.0:
        st.error(f"❌ 설정 오류: {len(tickers_list)}개 종목에 각각 {stock_alloc_pct}%씩 할당하면 총 {stock_alloc_pct * len(tickers_list)}%가 되어 씨드머니를 초과합니다. 종목 수나 할당 비중을 줄이세요.")
    else:
        with st.spinner("🔍 실시간 시세 및 안전 조건을 정밀 분석 중입니다..."):
            ...
            if use_market_ma20_filter:
                ...
                if warn_msg:
                    st.warning(f"🚨 **[기상 특보] 지수 폭락 경보 발령 중! ({', '.join(warn_msg)})** - 신규 진입이 차단됩니다.")
            ...
            for idx, t_code in enumerate(tickers_list):
                ...
                try:
                    ...
                    if len(hist) >= 20:
                        ...
                        prev_ma20 = float(hist['MA20'].iloc[-2])
                        ...
                        if daily_ret <= target_drop_rate:
                            ...
                            hold_stocks.append({
                                ...
                                '현재가': format_money(curr_price) + "원",
                                '당일등락률': f"{daily_ret:+.2f}%",
                                '상태': f"대기 (목표 타점 {target_drop_rate}% 미달)"
                            })
            ...
            if buy_orders:
                ...
                for b in buy_orders:
                    st.error(f"🎯 **{b['종목']} ({b['코드']})** | 현재가: **{b['현재가']}** ({b['당일등락률']}) ➔ **1호 요원 매수 발사!** (수량: **{b['추천수량']}** / 금액: {b['예상주문금액']})")
            else:
...
if run_btn or 'calculated' in st.session_state:
    ...
    with st.spinner("📡 [박가이버 사령부 V10.38] 정밀 체결 & 리스크 필터 적용 연산 중..."):
        ...
        if not kospi_df.empty: kospi_df['MA20'] = kospi_df['Close'].rolling(window=20).mean()
        if not kosdaq_df.empty: kosdaq_df['MA20'] = kosdaq_df['Close'].rolling(window=20).mean()
        ...
        for idx, ticker in enumerate(tickers_list):
            ...
            for date, row in df.iterrows():
                ...
                # --- 🌟 [업그레이드 3 & 4] 장중 High/Low 정밀 매도 및 수익 보존망 ---
                has_winner = False
                has_emergency_cut = False
                has_preserved_win = False
                
                for pos in positions:
                    if high > pos.get('max_price', pos['entry_price']):
                        pos['max_price'] = high
                    
                    # 목표가 도달 판정 (장중 고가 High 기준)
                    if ((high - pos['entry_price']) / pos['entry_price']) * 100 >= pos['target_ret']:
                        has_winner = True
                    
                    # 수익 보존망 판정 (+5% 도달 후 +3% 락온)
                    if profit_preserve_active:
                        if pos.get('max_price', pos['entry_price']) >= pos['entry_price'] * 1.05:
                            pos['safety_active'] = True
                        if pos.get('safety_active', False) and low <= pos['entry_price'] * 1.03:
                            has_preserved_win = True

                    # 비상 탈출 손절 판정 (장중 저가 Low 기준)
                    if emergency_cut_active:
                        if ((low - pos['entry_price']) / pos['entry_price']) * 100 <= emergency_threshold:
                            has_emergency_cut = True

                if has_winner or has_emergency_cut or has_preserved_win:
                    total_cycles += 1
                    total_cycles_all += 1
                    ...
                    for pos in positions:
                        ...
                        if has_winner:
                            ...
                            sell_price_exec = target_price if high >= target_price else close
                            trade_label = f"🎯 정상 복귀(+{pos['target_ret']:.0f}%)"
                        elif has_preserved_win:
                            sell_price_exec = pos['entry_price'] * 1.03
                            trade_label = "🛡️ 수익 보존(+3%)"
                        else:
                            stop_price = pos['entry_price'] * (1 + emergency_threshold/100)
                            sell_price_exec = stop_price if low <= stop_price else close
                            trade_label = f"🚨 비상 탈출({emergency_threshold:.0f}%)"

                        sell_gross = shares * sell_price_exec
                        ...
                        if profit_krw > 0:
                            if selected_strategy == '3tier':
                                ...
                                buyable_core = int((profit_krw * 0.20) // sell_price_exec)
                        ...
                        current_batch_trades.append({
                            ...
                            '청산단가': format_money(sell_price_exec) + "원", '매도금액': format_money(sell_amount_net) + "원",
                            '총수수료·세금': format_money(trade_fee_total) + "원",
                            '등락폭': f"{'+' if sell_price_exec >= pos['entry_price'] else ''}{format_money(sell_price_exec - pos['entry_price'])}원 ({ret:+.2f}%)",
                ...
                # --- 🌟 [업그레이드 2 & 3] 장중 Low 기반 맞춤 타점 정밀 매수 ---
                ...
                if low <= drop_target_price and len(positions) < max_agents:
                    ...
                    if ((not use_ma20_filter) or is_above_ma20) and market_safe and trend_safe:
                        ...
                        shares = max(int(agent_budget // drop_target_price), 1)
        ...
        if not combined_equity_df.empty:
            ...
            # 종목별 종합 성적표 (옥석 가리기 현황판 7열 완벽 복원)
            ...
            # 🌟 공식 매매 장부 (17열 테이블 완벽 복원)
            st.markdown("<div style='margin-top:25px;margin-bottom:8px;font-size:14px;font-weight:bold;color:#2c3e50;'>📜 박가이버 사령부 V10.38 공식 매매 장부 (익절=연분홍 / 손절=연파랑)</div>", unsafe_allow_html=True)
    ...
    st.info("👈 왼쪽 사이드바에서 종목과 조건 설정 후 [▶️ 박가이버 사령부 V10.38 작전 개시!] 버튼을 눌러주세요.")
